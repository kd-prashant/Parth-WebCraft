...
export default function Lanyard({
  ...
  return (
    <div className="lanyard-wrapper pointer-events-auto">
      <Canvas
...
function Band({
  ...
  const cardMap = useMemo(() => {
    ...
    const drawFitted = (img: HTMLImageElement | HTMLCanvasElement | ImageBitmap, rect: { x: number; y: number; w: number; h: number }) => {
      ...
      const dx = rx + (rw - dw) / 2;
      const dy = ry + (rh - dh) * 0.1; // Shifted up to prevent cropping the top of the head
      ctx.save();
      ctx.beginPath();
      ...
      <group position={[0, 4, 0]}>
        ...
        <RigidBody position={[2, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          ...
          <group
            ...
            onClick={() => window.open('https://www.linkedin.com/in/prashant-kandpal-1b4375282/', '_blank')}
          >
